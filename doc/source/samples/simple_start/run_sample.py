...
###############################################   load data
print("load data")
portf = eao.serialization.from_json(file_name=sample_file)
timegrid = eao.serialization.from_json(file_name=timegrid_file)
prices = {
    "price sourcing": 5.0 * np.sin(np.linspace(0.0, 6.0, timegrid.T)),
    "price sales a": np.ones(timegrid.T) * 2.0,
    "price sales b": 5.0 + 5.0 * np.sin(np.linspace(0.0, 6.0, timegrid.T)),
}

###############################################   visualization
print("create and write network graph to pdf file")
eao.network_graphs.create_graph(
    portf=portf, file_name=graph_file, title="Simple portfolio example"
)
