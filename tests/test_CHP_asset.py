...
class CHPAssetTest(unittest.TestCase):
    def test_optimization(self):
        ...
        check = all(
            x_power[np.sign(op.c[: timegrid.T]) == -1]
            + a.conversion_factor_power_heat * x_heat[np.sign(op.c[: timegrid.T]) == -1]
            == op.u[: timegrid.T][np.sign(op.c[: timegrid.T]) == -1]
        ) and all(
            x_power[np.sign(op.c[: timegrid.T]) == 1]
            + a.conversion_factor_power_heat * x_heat[np.sign(op.c[: timegrid.T]) == 1]
            == 0
    ...
    def test_max_share_heat(self):
        ...
        node_heat = eao.assets.Node("node_heat")
        timegrid = eao.assets.Timegrid(
            dt.date(2021, 1, 1), dt.date(2021, 1, 3), freq="h"
        )
        heat_price = -5
        chp = eao.assets.CHPAsset(
            name="CHP",
            nodes=(node_power, node_heat),
            min_cap=0,
            max_cap=10.0,
        ...
        power = eao.assets.SimpleContract(
            name="powerMarket",
            price="priceP",
            nodes=node_power,
            min_cap=-100,
        ...
        heat = eao.assets.SimpleContract(
            name="heatMarket",
            price="priceH",
            nodes=node_heat,
            min_cap=-100,
    ...
    def test_min_cap_vector(self):
        ...
        sc_power = eao.assets.SimpleContract(
            name="SC_power",
            price="rand_price",
            nodes=node_power,
            min_cap=-20.0,
        ...
        a = eao.assets.CHPAsset(
            name="CHP", nodes=(node_power, node_heat), min_cap=min_cap, max_cap=max_cap
        )
        ...
        self.assertAlmostEqual(
            np.abs(
                timegrid.values_to_grid(min_cap)
                - x_power
                + a.conversion_factor_power_heat * x_heat
            ).max(),
            0.0,
            2,
        )
    ...
    def test_max_cap_vector(self):
        ...
        self.assertAlmostEqual(
            np.abs(
                res.x[: timegrid.T]
                + a.conversion_factor_power_heat * res.x[timegrid.T : 2 * timegrid.T]
                - timegrid.values_to_grid(max_cap)
            ).sum(),
            0.0,
            5,
        )
    ...
    def test_minruntime(self):
        ...
        start_variables = res.x[3 * timegrid.T :]
        self.assertAlmostEqual(res.value, 10 * 1000.0 + 10 * (-5) - 1.0, 4)
        # min run time 20
        a = eao.assets.CHPAsset(
            name="CHP",
            price="price",
        ...
        disp_variables = res.x[0 * timegrid.T : 1 * timegrid.T]
        start_variables = res.x[3 * timegrid.T :]
        self.assertAlmostEqual(on_variables.sum(), 20.0, 4)
        # 10 times full load, 10 time min load
        self.assertAlmostEqual(res.value, 10 * 10 * 100.0 - 10 * 1 + 20 * (-5) - 1.0, 4)
        ...
        disp_variables = res.x[0 * timegrid.T : 1 * timegrid.T]
        start_variables = res.x[3 * timegrid.T :]
        self.assertAlmostEqual(on_variables.sum(), 15.0, 4)
        # 10 times full load, 10 time min load, NO start!
        self.assertAlmostEqual(res.value, 10 * 10 * 100.0 - 5 * 1 + 15 * (-5), 4)
    ...
    def test_gas_consumption(self):
        ...
        self.assertAlmostEqual(check, 190.0, 4)
        check = out["dispatch"]["CHP (node_heat)"].sum()
        self.assertAlmostEqual(check, 0.0, 4)
    ...
    def test_min_take(self):
        ...
        self.assertAlmostEqual(
            (
                disp_power[2 * 24 : 4 * 24]
                + conversion_factor_power_heat * disp_heat[2 * 24 : 4 * 24]
            ).sum(),
            20,
            5,
        )
    ...
    def test_freq_conversion(self):
        ...
        old_value = 12 * 30
        new_value = eao.basic_classes.convert_time_unit(
            old_value, old_freq="h", new_freq="15min"
        )
        ...
        old_value = 16 * 30
        new_value = eao.basic_classes.convert_time_unit(
            old_value, old_freq="30min", new_freq="min"
        )
        self.assertAlmostEqual(new_value - old_value * 30, 0, 5)

        old_value = 22 * 30
        new_value = eao.basic_classes.convert_time_unit(
            old_value, old_freq="min", new_freq="h"
        )
    ...
    def test_start_and_shutdown_ramp2(self):
        ...
        disp_true = (
            start_ramp_lower_bounds[1:]
            + min_runtime * [min_cap]
            + list(reversed(shutdown_ramp_lower_bounds))
            + [0.0] * 6
        )
        ...
        shutdown_variables_true[
            len(start_ramp_lower_bounds)
            - 1
            + min_runtime
            + len(shutdown_ramp_lower_bounds)
        ] = 1
        self.assertAlmostEqual(
            abs(shutdown_variables_true - shutdown_variables).sum(), 0, 4
        )
        ...
        prices["price"][
            5 : 5 + len(start_ramp_lower_bounds) + len(shutdown_ramp_lower_bounds)
        ] = -1
        ...
        disp_true = (
            ...
            + list(reversed(shutdown_ramp_upper_bounds))
            + [0.0]
            * (
                timegrid.T
                - len(start_ramp_lower_bounds)
        ...
        shutdown_variables_true = np.zeros(timegrid.T)
        shutdown_variables_true[
            5 + len(start_ramp_lower_bounds) + len(shutdown_ramp_lower_bounds)
        ] = 1
        self.assertAlmostEqual(
            abs(shutdown_variables_true - shutdown_variables).sum(), 0, 4
        )
    ...
    def test_start_and_shutdown_ramp3(self):
        ...
        prices = {"price": 10 * np.ones(timegrid.T)}
        prices["price"][
            5 : 5 + int(np.ceil(len(start_ramp) / 4) + np.ceil(len(shutdown_ramp) / 4))
        ] = -1
        ...
        # negative, otherwise asset is off
        converted_time_rounded = int(
            np.ceil(
                eao.basic_classes.convert_time_unit(
                    value=1, old_freq=timegrid.freq, new_freq=ramp_freq
                )
            )
        )
        start_ramp_padding_size = int(
            np.ceil(len(start_ramp) / converted_time_rounded) * converted_time_rounded
        ) - len(start_ramp)
        start_ramp_padded = start_ramp + [start_ramp[-1]] * start_ramp_padding_size
        start_ramp_interpolated = np.reshape(
            start_ramp_padded, (-1, converted_time_rounded)
        ).mean(axis=1)

        shutdown_ramp_padding_size = int(
            np.ceil(len(shutdown_ramp) / converted_time_rounded)
            * converted_time_rounded
        ) - len(shutdown_ramp)
        shutdown_ramp_padded = (
            shutdown_ramp + [shutdown_ramp[-1]] * shutdown_ramp_padding_size
        )
        shutdown_ramp_interpolated = np.reshape(
            shutdown_ramp_padded, (-1, converted_time_rounded)
        ).mean(axis=1)

        disp_res = power_variables + a.conversion_factor_power_heat * heat_variables
        disp_true = (
            [0] * 5
            + list(start_ramp_interpolated)
            + list(reversed(shutdown_ramp_interpolated))
            + [0]
            * (
                timegrid.T
                - len(start_ramp_interpolated)
                - len(shutdown_ramp_interpolated)
                - 5
            )
        )
        self.assertAlmostEqual(abs(disp_res - disp_true).sum(), 0, 4)
        ...
        shutdown_variables_true = np.zeros(timegrid.T)
        shutdown_variables_true[
            5 + len(start_ramp_interpolated) + len(shutdown_ramp_interpolated)
        ] = 1
        self.assertAlmostEqual(
            abs(shutdown_variables_true - shutdown_variables).sum(), 0, 4
        )
    ...
    def test_start_and_shutdown_ramp_heat_2(self):
        ...
        d = eao.assets.SimpleContract(
            name="heatMarket", nodes=node_heat, min_cap=-100, max_cap=100
        )
        prices = {
            "price": 0.0 * np.ones(timegrid.T),
            "priceGas": 10 * np.ones(timegrid.T),
        }
        #                 'heat_demand': np.zeros(timegrid.T)}
        #        prices['heat_demand'][10:20] = -1
...
class CHPAssetTest_with_threshhold(unittest.TestCase):
    ...
    def test_optimization(self):
        ...
        demand = eao.assets.SimpleContract(
            name="demand", nodes=node_power, min_cap="demand", max_cap="demand"
        )
        prices = {
    ...
    def test_optimization_vectors(self):
        ...
        self.assertAlmostEqual(
            np.abs(
                costs[disp < prices["ml_t"]] + prices["ml_c"][disp <= prices["ml_t"]]
            ).sum(),
            0,
            5,
        )
    ...
    def test_isrunning(self):
        ...
        node_heat = eao.assets.Node("node_heat")
        timegrid = eao.assets.Timegrid(
            dt.date(2020, 2, 1), dt.date(2020, 2, 2), freq="2h"
        )
        # define a CHP with extra costs when dispatch is below threshhold of 4
        a = eao.assets.CHPAsset_with_min_load_costs(
            name="CHP",
        ...
        self.assertAlmostEqual(
            np.abs(costs[(disp < 7 * 2) & (disp > 0)] + 2).sum(), 0, 5
        )
    ...
    def test_check_indexing(self):
        ...
        node_heat = eao.assets.Node("node_heat")
        timegrid = eao.assets.Timegrid(
            dt.date(2020, 2, 1), dt.date(2020, 2, 2), freq="h"
        )
        # define a CHP with extra costs when dispatch is below threshhold of 4
        a = eao.assets.CHPAsset_with_min_load_costs(
            name="CHP",
        ...
        gas = eao.assets.SimpleContract(
            name="gas", nodes=node_gas, min_cap=-1000, max_cap=1000
        )
        prices = {
            "price": np.zeros(timegrid.T),
            "demand": -np.linspace(2, 10, timegrid.T),
...
class CHPAssetTest_no_heat(unittest.TestCase):
    ...
    def test_gas_consumption_no_heat(self):
        ...
        d = eao.assets.SimpleContract(
            name="heatMarket", price="priceGas", nodes=node_heat, min_cap=0, max_cap=0
        )
        prices = {
            "price": 50.0 * np.ones(timegrid.T),
            "priceGas": 0.1 * np.ones(timegrid.T),
...
class CHPAssetTest_with_PQ_polygon(unittest.TestCase):
    ...
    def test_check_polygon(self):
        ...
        poly = [[50.0, 50.0], [0.0, 0.0], [50.0, 0.0]]  # triangle, counter clockwise
        self.assertEqual(a._check_polygon(poly), -1)
        poly = [
            [0, 0],
            [0, 100],
    ...
    def test_build_polygon_asset_square(self):
        ...
        node_heat = eao.assets.Node("node_heat")
        timegrid = eao.assets.Timegrid(
            dt.date(2024, 1, 1), dt.date(2024, 1, 2), freq="h"
        )
        data = {
            "price": np.sin(np.linspace(0, 20 * np.pi, timegrid.T)),
            "heat": -50 * (np.linspace(0, 1, timegrid.T)) - 15,
        }
        # square
        #        [P, Q]
        poly = [[5, 15], [10, 15], [10, 25], [5, 25]]
        # no further restriction - instantaneous reaction to prices
        m = eao.assets.SimpleContract(
            name="market", price="price", nodes=node_power, min_cap=-1000, max_cap=1000
        )
        h = eao.assets.SimpleContract(
            name="heat", nodes=node_heat, min_cap="heat", max_cap="heat"
        )
        a = eao.assets.CHP_PQ_diagram(
            name="poly",
            nodes=(node_power, node_heat),
            min_cap=0.0,
            max_cap=500.0,  # generous, restr only from poly!
            conversion_factor_power_heat=0.2,
            pq_polygon=poly,
        )
        ...
        # power limits
        ind = op.mapping[
            (op.mapping["asset"] == "poly")
            & (op.mapping["node"] == "node_power")
            & (op.mapping["type"] == "d")
        ].index.values
    ...
    def test_build_polygon_asset_tetra(self):
        ...
        node_heat = eao.assets.Node("node_heat")
        timegrid = eao.assets.Timegrid(
            dt.date(2024, 1, 1), dt.date(2024, 1, 20), freq="h"
        )
        data = {
            "price": np.sin(np.linspace(0, 20 * np.pi, timegrid.T)),
        ...
        # square
        #        [P, Q]
        poly = [[5, 5], [10, 15], [8, 25], [6, 30]]
        poly_eps = [
            [4.99, 4.88],  # making it a bit bigger to avoid numerical issues
            [10.01, 14.99],
            [8.01, 25.01],
            [5.99, 30.03],
        ]
        ...
        for i in range(len(x)):
            ...
            self.assertTrue(
                poly_path.contains_point(point), msg=f"point {point} not in polygon"
            )
        return
    ...
    def test_build_polygon_asset_mixed_straight_tetra(self):
        ...
        # square
        #        [P, Q]
        poly = [[5, 5], [10, 5], [8, 25], [6, 30]]
        poly_eps = [
            [4.99, 4.99],  # making it a bit bigger to avoid numerical issues
            [10.01, 4.99],
            [8.01, 25.01],
            [5.99, 30.03],
        ]
    ...
    def test_polygon_basics(self):
        ...
        h1 = eao.assets.SimpleContract(
            name="kessel", nodes=node_heat, min_cap=0, max_cap=10, extra_costs=5
        )
        a = eao.assets.CHP_PQ_diagram(
            name="poly",
            nodes=(node_power, node_heat),
