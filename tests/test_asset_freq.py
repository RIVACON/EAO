...
class AssetFrequency(unittest.TestCase):
    ...
    def test_freq_simple_portfolio(self):
        ...
        self.assertAlmostEqual(
            disp.sum(axis=1).abs().sum(), 0.0, 5
        )  # in each hour net zero
        for t in a1.timegrid.restricted.timepoints:
            assert len(disp[disp.index.date == t.date()]) == 24
            all(
                disp[disp.index.date == t.date()]["SC_2"]
                == disp[disp.index.date == t.date()].iloc[0, 1]
            )
        assert all(disp["SC_2"].iloc[0:23].values.round(3) == 10.0)
        assert all(disp["SC_2"].iloc[24:].values.round(3) == -5.0)
        assert out["DCF"]["SC_2"].sum().round(4) == 60000.0
    ...
    def test_freq_ST_LT_storage(self):
        ...
        prices = {
            "p1": 10 * np.sin(np.linspace(0, 2 * np.pi, timegrid.T))
            + 10 * np.sin(np.linspace(0, 7 * 2 * np.pi, timegrid.T))
        }
        ...
        for t in lt.timegrid.restricted.timepoints:
            ...
            weeks[weeks == 54] = 1
            assert all(
                disp[weeks == t_week]["lt"]
                == disp[disp.index.date == t.date()].iloc[0, 2]
            )
    ...
    def test_freq_multi_commodity(self):
        ...
        out = eao.io.extract_output(portf, op, res, prices)
        fact = (
            portf.assets[2].factors_commodities[1]
            / portf.assets[2].factors_commodities[0]
        )
        disp = out["dispatch"]
        check = disp["CHP (heat)"] / disp["CHP (power)"]
        self.assertAlmostEqual((check - fact).sum(), 0.0, 4)
