...
class OptimizationTests(unittest.TestCase):
    ...
    def test_trivial_slp(self):
        ...
        a2 = eao.assets.SimpleContract(
            name="SC_2", price="price1", nodes=node1, min_cap=-5.0, max_cap=10.0
        )
        a3 = eao.assets.SimpleContract(
            name="SC_3",
            price="price2",
        ...
        a4 = eao.assets.Transport(
            name="Tr", costs=0.0, nodes=[node1, node2], min_cap=0.0, max_cap=1.0
        )

        # starting point with full set of prices across the grid
        price1 = {"start": Start, "values": 1.0}
        price2 = {"start": Start, "values": 20.0}
    ...
    def test_fixing_results(self):
        ...
        pricesB = {
            "rand_price_2": np.sin(np.linspace(0, 10, timegrid.T)),
        ...
        opC = portf.setup_optim_problem(
            pricesB, timegrid, fix_time_window=fix_time_window
        )
...
class SplitOptimizationTests_paralel(unittest.TestCase):
    def test_same_same_parallel(self):
        ...
        t0 = time.perf_counter()
        resC = opB.optimize_parallel(max_workers=2)
        outC = eao.io.extract_output(portf, opB, resC)
        timings["split_optim_parallel"] = time.perf_counter() - t0 + t_op

        ### call via wrapper
        t0 = time.perf_counter()
        outD = eao.optimize(portf, timegrid, pricesA, split_interval_size="D", 
                            parallelize_intervals=True, max_workers=2)
    ...
    def test_improvements(self):
        return ### this test is only for manual performance comparison, not for correctness
        node1 = eao.assets.Node("node_1")
        node2 = eao.assets.Node("node_2")
