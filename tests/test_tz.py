...
class TimeZones(unittest.TestCase):
    def test_opt_tz(self):
        ...
        buy = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=1, nodes=node1a, start=dt.date(2021, 1, 2)
        )
        ...
        )  # no change in summer/winter time --> same result
    ...
    def test_contract_with_max_take(self):
        ...
        tgNaive = eao.assets.Timegrid(
            Start, End, freq="h", main_time_unit="h", timezone=None
        )
        self.assertEqual(tgNaive.T, h_naive)
        tgCET = eao.assets.Timegrid(
            Start, End, freq="h", main_time_unit="h", timezone="CET"
        )
        ...
        min_take["end"] = End
        min_take["values"] = -10.0

        prices = {"buy": np.ones(tgCET.T), "sell": 10.0 * np.ones(tgCET.T)}
        buy = eao.assets.SimpleContract(name="buya", price="buy", max_cap=1, nodes=node)
        sell = eao.assets.Contract(
            name="sell", price="sell", min_cap=-1, min_take=min_take, nodes=node
        )
    ...
    def test_tz_serialization(self):
        ...
        min_take["values"] = -10.0 * np.ones(len(min_take["start"]))
        sell = eao.assets.Contract(
            name="sell", price="sell", min_cap=-1, min_take=min_take, nodes=node
        )
        ss = eao.serialization.to_json(sell)
        c2 = eao.serialization.from_json(ss)
    ...
    def test_tz_to_grid(self):
        ...
        End = dt.date(2020, 10, 26)
        tgCET = eao.assets.Timegrid(
            Start, End, freq="10min", main_time_unit="h", timezone="CET"
        )
        mybuy = {}
        ...
        p2 = eao.serialization.from_json(ss)
    ...
    def test_cast_from_df_tz(self):
        ...
        tg_dah = eao.basic_classes.Timegrid(
            start=Start, end=End, freq="h", timezone="CET"
        )
        #### cast to grid: there was an issue here:
        p = tg_dah.prices_to_grid(data)
        self.assertAlmostEqual(
            data.loc["2024-02-01 00:00:00+01:00", "id"],
            p.loc["2024-02-01 00:00:00+01:00", "id"],
            3,
        )
        self.assertAlmostEqual(
            data.loc["2024-02-01 02:00:00+01:00", "id"],
            p.loc["2024-02-01 02:00:00+01:00", "id"],
            3,
        )
