...
class PortfolioTests(unittest.TestCase):
    ...
    def test_portfolio_with_transport(self):
        ...
        check = (
            out["dispatch"]["Tr (node_2)"] + out["dispatch"]["SC_3 (node_2)"]
        ).sum()
        self.assertAlmostEqual(check, 0.0, 5)
        check = (
            out["dispatch"]["Tr (node_1)"]
            + out["dispatch"]["SC_1 (node_1)"]
    ...
    def test_various_vars_in_mapping(self):
        ...
        s = eao.assets.SimpleContract(
            name="s", nodes=node2, min_cap=-100, max_cap=100, price="P"
        )

        take = {
            "start": pd.date_range(
                start=pd.Timestamp(1980, 1, 1), end=pd.Timestamp(1982, 1, 1), freq="MS"
            ),
            "end": pd.date_range(
                start=pd.Timestamp(1980, 2, 1), end=pd.Timestamp(1982, 2, 1), freq="MS"
            ),
        }
        take["values"] = np.ones(len(take["start"])) * 22
        t = eao.assets.ExtendedTransport(
            name="trans",
            max_cap=100,
            max_take=take,
            efficiency=0.9,
        ...
        price = {"Z": np.zeros(tg.T), "P": np.ones(tg.T) * 1}
    ...
    def test_portf_min_cap_from_price_DF(self):
        ...
        self.assertAlmostEqual(abs(disp[0:9]).sum(), 0.0, 5)
        self.assertAlmostEqual(abs(disp[19:]).sum(), 0.0, 5)
        self.assertAlmostEqual(abs(disp[9:19] - prices["CAP"][9:19]).sum(), 0.0, 5)
...
class StructuredAssetsTests(unittest.TestCase):
    ...
    def test_structured_asset(self):
        ...
        # a3.set_timegrid(timegrid)
        prices = {
            "rand_price_1": (np.ones(timegrid.T) * 1.0).tolist(),
            "rand_price_2": (np.ones(timegrid.T) * 10.0).tolist(),
        }
    ...
    def test_linked_asset(self):
        ...
        demand = eao.assets.SimpleContract(
            name="demand", nodes=node_power, min_cap=-10.0, max_cap=-10.0
        )
        ...
        self.assertAlmostEqual(
            np.abs(out["DCF"]["extra_CHP"].values + 75.0).sum(), 0, 4
        )
