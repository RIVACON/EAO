...
from eaopack.serialization import from_json
from eaopack.assets import Transport, ExtendedTransport


def create_graph(
    portf: Portfolio, file_name: str = None, title=None, no_image_output=False
):
    ...
    if not no_image_output:
        nx.draw_networkx_nodes(
            G, pos, node_color=color_map, node_size=node_sizes, alpha=0.7
        )
        ...
        if title is None:
            title = "Network graph for portfolio"
        # pyplot version
        plt.title(title)
        plt.axis("off")
